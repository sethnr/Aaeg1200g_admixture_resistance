...
#get significant admixed pops based on whole genome f3
sigpopsP3 <- unique(pop3sum$p3[pop3sum$f3sig])
...
ggplot(subset(pop3results,!p1 %in% sigpopsP3 & !p2 %in% sigpopsP3 & !p3 %in% sigpopsP3 & !set %in% sigsets),aes(x=mid,y=f3z,group=set,color=parent)) + 
  geom_line() + 
...
```
## Warning: Removed 482015 row(s) containing missing values (geom_path).
```
...
countrytotals$admixed3P <- c("not tested","not admixed","admixed")[countrytotals$f3sig+2]

# ggplot() +
#     geom_polygon(data = world, aes(x=long, y = lat, group=group), fill='grey',color="black",size=0.2) +
#     geom_point(data=countrytotals, aes(x=lon, y=lat, fill=admixed3P,size=n), shape=21,inherit.aes=F) +
...
```r
ggplot(subset(sigmixes,!p1 %in% sigpopsP3 & !p2 %in% sigpopsP3),aes(x=p1,y=p2,fill=f3zmin)) + 
  geom_raster() + facet_wrap("p3") + coord_fixed() +
  theme(axis.text.x=element_text(angle=45,hjust=1),axis.title=element_blank())
```
...
```
## Error in eval(e, x, parent.frame()): object 'admixed' not found
```
