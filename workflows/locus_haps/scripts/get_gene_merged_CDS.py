...
genes = allel.gff3_to_recarray(gff,attributes=["gene_id"])
cds = genes[np.logical_and(genes['type']=='CDS', genes['gene_id']==geneid)]

ranges = [i for i in zip(cds['start'],cds['end'])]
...
outbed = np.array([i for i in zip([cds['seqid'][1]] * len(merged),
            [i[0] for i in merged],
            [i[1] for i in merged],
            [cds['gene_id'][1]] * len(merged))],
