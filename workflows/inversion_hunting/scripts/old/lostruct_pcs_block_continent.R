...
write(paste("contfile =",contfile),file=stderr())
write(paste("cont =",cont),file=stderr())
...
conttab <- read.table(contfile,header=T, sep="\t")
samples <- conttab$sample[tolower(conttab$contgroup)==cont]
